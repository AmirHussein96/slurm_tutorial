...
def remove_dot(text):
    ...
    res = []
    for word in words:
        # remove the dot if it is not part of a number
...
def east_to_west_num(text):
    eastern_to_western = {"٠": "0", "١": "1", "٢": "2", "٣": "3", "٤": "4", "٥": "5",
                          "٦": "6", "٧": "7", "٨": "8", "٩": "9", "٪": "%", "_": " ", "ڤ": "ف", "|": " "}
...
def digit2num(text, dig2num=False):
    ...
    res = re.findall(r'[^\u0600-\u06FF%]+', text)  # search for digits
    for match in res:
...
def main():
    ...
    # input transcription file with format <id> <text>
